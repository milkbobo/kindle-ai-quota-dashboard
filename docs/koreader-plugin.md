...
- **每天定时息屏**：菜单 → 工具 → AI 额度 →「每天定时息屏：HH:MM」里设时间（例如 18:30；**留空=关闭**，
  默认关闭）。到点时如果**面板正开着**，会先画一屏黑底「已息屏」再让设备真休眠——
  墨水屏断电会保留最后一帧，不自己画一屏的话看起来就像没关掉。
  - 只在面板开着时动手：正在看书时不会被强制息屏。
  - 醒来（按电源键/开套）后**不会立刻又睡回去**：如果目标是睡着时跨过去的，当天标记为已完成。
  - 准点靠一个定时器；休眠期间单调时钟不走，所以每次刷新（5 分钟）还会核一次钟点兜底。
  - 想临时改时间/测试可以用环境变量 `AIQUOTA_DAILY_SLEEP=18:30`（优先于菜单设置）。
- **每天定时开屏（硬件 RTC 唤醒）**：菜单 →「每天定时开屏：HH:MM」设时间（例如 09:00；留空=关闭，
  ...
  **设备睡着也会在指定时刻自己醒过来**，醒来后面板自动重画。
  - **默认只在工作日开屏**（周六周日跳过，闹钟会直接排到下周一）：菜单 →「开屏只在工作日（周一至周五）」可关掉。
