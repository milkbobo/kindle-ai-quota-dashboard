...
启动后立即刷新上传一次，每轮结束后等待 **2 分钟**再执行。失败时输出日志，下轮重试，各轮不会重叠。按 `Ctrl+C` 停止。

**开机自动后台运行（可选）**：仓库自带 `scripts/watch-loop.cjs`，它解决三件事——

1. **防重复**：启动先看 `state/kindle.json` 是否 5 分钟内更新过（说明已有实例在跑），是就直接退出，
   不会起第二个实例去并发采集/推送；
2. **自愈**：常驻执行 `npm run watch`，万一崩了或被关掉，30 秒后自动重启；
3. **留痕**：所有输出追加到 `logs/refresh-watch.log`（`logs/` 已在 `.gitignore` 里）。

放进开机启动即可：`Win+R` → `shell:startup` → 在打开的目录里放一个指向它的启动项
（Windows 上可用一个两行的 `.vbs` 无窗口启动，见仓库 `scripts/start-watch.vbs` 示例）。
手动启动等价命令：`npm run watch:loop`；停止就在任务管理器里结束 `node.exe`。
